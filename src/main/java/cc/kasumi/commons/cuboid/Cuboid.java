...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    public Cuboid(CuboidSelection selection1, CuboidSelection selection2) {
        if (!selection1.getWorld().equals(selection2.getWorld())) throw new IllegalArgumentException("Locations must be on the same world");
        this.worldName = selection1.getWorld();
        this.x1 = Math.min(selection1.getX(), selection2.getX());
        this.y1 = Math.min(selection1.getY(), selection2.getY());
        this.z1 = Math.min(selection1.getZ(), selection2.getZ());
        this.x2 = Math.max(selection1.getX(), selection2.getX());
        this.y2 = Math.max(selection1.getY(), selection2.getY());
        this.z2 = Math.max(selection1.getZ(), selection2.getZ());
    }
    ...
    public Location getPointA() {
        return new Location(Bukkit.getWorld(this.worldName), this.x1, this.y1, this.z1);
    }

    public Location getPointB() {
        return new Location(Bukkit.getWorld(this.worldName), this.x2, this.y2, this.z2);
    }
