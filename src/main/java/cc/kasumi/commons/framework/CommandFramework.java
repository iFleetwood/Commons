package cc.kasumi.commons.framework;
	...
	public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if (!command.permission().equals("") && !sender.hasPermission(command.permission() + "." + command.name().toLowerCase()) && !sender.hasPermission(command.permission() + ".*")) {
